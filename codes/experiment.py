...
def run_command(lies_path, k_hop, ns, rw=False):
    if not rw:
        command = f"""python -u codes/run.py --cuda --do_train --do_valid --data_path data/converted --model TransE -n """\
            f"""256 -b 1024 -d 1000 -g 24.0 -a 1.0 -lr 0.005 --max_steps 1000 -save models/{lies_path.split('/')[-1]}""" \
            f""" --test_batch_size 16 -khop {k_hop} --log_steps 100 -ns {ns} --lies {lies_path} -save_results 1"""\
            """ --results /var/scratch/yan370/SANSOL/results.txt --warm_up_steps 0"""
    else:
        command = f"""python -u codes/run.py --cuda --do_train --do_valid --data_path data/converted --model TransE -n """\
            f"""256 -b 1024 -d 1000 -g 24.0 -a 1.0 -lr 0.005 --max_steps 1000 -save models/{lies_path.split('/')[-1]}""" \
            f""" --test_batch_size 16 -khop {k_hop} --log_steps 100 -ns {ns} --lies {lies_path} -save_results 1"""\
            """ --results /var/scratch/yan370/SANSOL/results.txt -nrw 100 --warm_up_steps 0"""
...
def main(skip_to_rpns=None, skip_to_id=None, k_hop=3, corr=False, rw=False):
    ...
    rpns_rates = [5, 10, 15, 20, 32, 40, 50, 60, 70, 80, 90, 100]
